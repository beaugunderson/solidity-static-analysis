...
staticAnalysisRunner.prototype.run =
  function (compilationResult, toRun, callback) {
    var modules = toRun.map((i) => {
...
staticAnalysisRunner.prototype.runWithModuleList =
  function (compilationResult, modules, callback) {
    // Also provide convenience analysis via the AST walker.
    var walker = new AstWalker();

    for (var k in compilationResult.sources) {
      walker.walk(compilationResult.sources[k].AST, {
        '*': function (node) {
    ...
    var reports = modules.map(function (item) {
      return {
        name: item.name,
